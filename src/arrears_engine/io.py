...
def load_tenants(workbook_path:str, sheet: str) -> List[Tenant]:
    ...
    required = ["Lease_ID", "Tenant_Name", "Monthly_Rent", "Due_Day", "Unit", "Contact_Email", "Property_Name"]
        ...
        tenants.append(
            Tenant(
                ...
                property_name=(
                        ...
                        if pd.notna(row["Property_Name"])
                        else "UNKNOWN"
                    ),
                manager_name=(
                        str(row["Manager_Name"]).strip()
                        if pd.notna(row["Manager_Name"])
                        else "UNKNOWN"
                    ),
            )
        )
...
def load_ledger(workbook_path: str, sheet: str) -> List[LedgerEntry]:
    ...
    for _, row in df.iterrows():
        ...
        if entry_type not in {"charge", "payment"}:
            raise ValueError(f"Invalid Entry_Type '{row['Entry_Type]']}'. Use 'charge' or 'payment'.")
